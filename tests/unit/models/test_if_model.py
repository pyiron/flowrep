...
def _make_input_edges(cases, else_case=None):
    edges = {
        model.TargetHandle(node=case.body.label, port="x"): model.InputSource(
            port="inp"
        )
        for case in cases
    }
    if else_case is not None:
        edges[model.TargetHandle(node=else_case.label, port="x")] = model.InputSource(
            port="inp"
        )
    return edges
...
def _make_output_edges(cases, else_case=None):
    sources = [model.SourceHandle(node=case.body.label, port="y") for case in cases]
    if else_case is not None:
        sources.append(model.SourceHandle(node=else_case.label, port="y"))
...
class TestIfNodeInputEdgesValidation(unittest.TestCase):
    ...
    def test_input_edges_can_target_else(self):
        ...
        node = model.IfNode(
            ...
            else_case=else_case,
            input_edges={
                model.TargetHandle(node=else_case.label, port="x"): model.InputSource(
                    port="inp"
                ),
            },
            output_edges_matrix=_make_output_edges(cases, else_case),
    ...
    def test_roundtrip_multiple_cases(self):
        ...
        for mode in ["json", "python"]:
            with self.subTest(mode=mode):
                ...
                self.assertEqual(
                    len(restored.output_edges_matrix[model.OutputTarget(port="out")]), 4
                )
        ...
        with self.assertRaises(pydantic.ValidationError) as ctx:
            model.IfNode(
                ...
                output_edges_matrix={
                    model.OutputTarget(port="out"): [
                        model.SourceHandle(node=cases[0].body.label, port="y"),
                        model.SourceHandle(node=else_case.label, port="nonexistent"),
                    ]
                },
            )
        exc_str = str(ctx.exception)
