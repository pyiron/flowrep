...
class TestDiscriminatedUnionRoundtrip(unittest.TestCase):
    ...
    def test_atomic_roundtrip_preserves_defaults(self):
        data = {
            "type": "atomic",
            "reference": reference_dict(inputs_with_defaults=["b"]),
            "inputs": ["a", "b"],
            "outputs": ["c"],
        }
        ...
        # Full roundtrip
        dumped = pydantic.TypeAdapter(union.RecipeDiscrimination).dump_python(
            node, mode="json"
        )
        restored = pydantic.TypeAdapter(union.RecipeDiscrimination).validate_python(
            dumped
        )
    ...
    def test_workflow_roundtrip_preserves_defaults(self):
        data = {
            "type": "workflow",
            "reference": reference_dict(inputs_with_defaults=["x"]),
            "inputs": ["x"],
            "outputs": [],
            "nodes": {},
            "input_edges": {},
...
class TestNodesTypeAlias(unittest.TestCase):
    ...
    def test_valid_nodes_dict(self):
        ...
        nodes_data = {
            "step1": {
                "type": "atomic",
                "reference": reference_dict(qualname="f"),
                "inputs": [],
                "outputs": ["x"],
            },
            "step2": {
                "type": "atomic",
                "reference": reference_dict(qualname="g"),
                "inputs": ["x"],
                "outputs": [],
            },
        }
...
class TestNestedUnionResolution(unittest.TestCase):
    ...
    def test_deeply_nested_workflows(self):
        ...
        innermost = {
            "type": "atomic",
            "reference": reference_dict(qualname="leaf"),
            "inputs": ["x"],
            "outputs": ["y"],
        }
        middle = {
